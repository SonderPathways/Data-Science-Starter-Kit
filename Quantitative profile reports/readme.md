# Quantitative Profile Reports

In the current state of the Pathways project (June 1, 2026), the report creation process has been fully integrated into the online platform. This means that, through a combination of pipelines for performing the necessary data transformations, along with a series of specific views on the frontend, highly standardized reports are now available for each segmentation/geography.

However, it's important to note that in some situations requiring a high level of report customization, it might be faster or more flexible to create an external report using the templates available here. These templates were originally created as the final reports published during the platform's development (2024 to 2025).

The reports combine Python code for structuring and exporting, as well as all the necessary R code for performing the respective statistical calculations, thus maintaining methodological consistency with the project's overall standard.
